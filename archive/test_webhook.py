"""
Sends a fake webhook POST to our deployed Fly.io endpoint to verify it
processes a Sierra webhook payload correctly. Uses Leilani's lead id (2443063)
since we know her record well.

Reads WEBHOOK_SECRET from .env (same value as `fly secrets list` shows set).
Override the target with WEBHOOK_URL in .env to test another deployment.
"""
...
WEBHOOK_URL = os.environ.get(
    "WEBHOOK_URL", "https://sierra-fub-webhook.fly.dev/sierra-webhook"
)
...
if r.status_code == 200:
    ...
    print("\nFAIL - X-Webhook-Secret mismatch. Check `fly secrets list` vs .env.")
else:
